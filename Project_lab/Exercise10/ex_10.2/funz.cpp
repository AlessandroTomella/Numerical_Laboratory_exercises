...
Random :: Random(int rank){
 int seed[4];
   ...
   ifstream Primes("Primes");
   if (Primes.is_open()){
	for(int i = 0; i <= rank*5; i++) Primes >> p1 >> p2 ;
 ...
 ifstream input("seed.in");
  ...
  cout << "Primes for process " << rank << ": " << p1<< " " << p2 << endl;
return;
}
...
double L = 0;
//cout << "coordinate: " << x.at(v.at(0)) << " "<< y.at(v.at(0)) << endl;
//cout << "coordinate: " << x.at(v.size()-1) << " "<< y.at(v.size()-1) << endl;
...
SimAnneal :: SimAnneal(int init_shuffle, double pmutation, int rank) : Random(rank){
...
void Individual :: PrintInd(int file){
 ...
 ofstream print;
 if(geometry) print.open("Circlepath"+to_string(file)+".dat");
 else print.open("Squarepath"+to_string(file)+".dat");
