...
double epot_BlockMean = 0, ekin_BlockMean= 0, etot_BlockMean = 0, temp_BlockMean = 0;	//delle diverse quantità medie con
double epot_BlockVar = 0, ekin_BlockVar= 0, etot_BlockVar = 0, temp_BlockVar = 0;	//incertezza statistica
...
double gr[gr_bin];		// funzione g(r)
double ave_gr[gr_bin];		// media nel blocco di g(r)
double gr_BlockMean[gr_bin];	// media dei blocchi
double gr_BlockVar[gr_bin];	// varianza dei blocchi
...
bool ask(void);			// - chiede se si vuole iniziare la simulazione con quella temperatura
...
//void Reset(void);		// - ricalcola le posizioni vecchie con le nuove velocità
				//Il ciclo così costruito si ripete fino a che la funzione ask() non riceve una risposta affermativa
				//e in tal caso la simulazione inizia.
