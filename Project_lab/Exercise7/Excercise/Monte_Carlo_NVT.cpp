...
//Read input informations
  ...
  cout << "Number of steps in the equilibrations phase = " << eqstep << endl;
  cout << "Number of bins of g(r) = " << nbins << endl << endl;
  ReadInput.close();
...
//Read initial configuration
     ...
     ReadConf.close();
 /*
    cout << "Read initial configuration from config.0" << endl;
...
//update of the histogram of g(r)
	...
	if(dr >= (k-2)*bin_size && dr< (k-1)*bin_size) {
		walker[k] += 2;
		break;		
		}
	}
...
// contribution to energy and virial
  ...
  for(int k = igofr; k < igofr + nbins; k++) {
     dv = 4.0*pi/3.0 *(pow((k+1)*bin_size,3) - pow(k*bin_size, 3));
     walker[k] = walker[k]/(rho*npart*dv);
  } 
...
void Averages(int iblk) //Print results for current block
{
   ...
   Gofr.open("WorkInProg/gofr." + to_string(iblk));
   ...
   err_press=Error(glob_av[iw],glob_av2[iw],iblk);
    ...
    for(int k = igofr; k < igofr + nbins; k++) {
      ...
      glob_av2[k] += (blk_av[k]/blk_norm) * (blk_av[k]/blk_norm);
      err[k] = Error(glob_av[k],glob_av2[k],iblk);
      }
...
//g(r)   
    for(int k = igofr; k < igofr + nbins; k++) { 
    Gofr << iblk <<  " " << bin_size*(k-2) << " " << blk_av[k]/blk_norm << endl;
    }

    if(iblk == nblk){
    cout << "Writing final g(r)..." << endl;
    Gave.open("RadFunc/" + name + "/output.gave.0");
 	for(int k = igofr; k < igofr + nbins; k++) { 
    	Gave << bin_size*(k-2) << " " << glob_av[k]/(double)nblk << " " << err[k] << endl;
    	}
    Gave.close();
    }
    cout << "----------------------------" << endl << endl;
