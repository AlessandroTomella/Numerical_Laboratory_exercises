...
{
	return 1.0;		// Constant 
	//return exp(-0.5*v*v);	// Gaussian
	//return exp(-0.5*(v - mu)*(v - mu)/(sigma*sigma)) + exp(-0.5*(v + mu)*(v + mu)/(sigma*sigma));	// Sum of Gaussians
	//return 1.0/(1+v*v);	// Lorentzian
//Potrebbe servire la normalizzazione (copia eventualmente in _second):
//1.0/sqrt(sigma*sigma * PI * (2 + exp(-mu*mu/(sigma*sigma))))
...
{
	return 0;		// Constant
	//return v*v*exp(-0.5*v*v) - exp(-0.5*v*v);	// Gaussian
	//return exp(-0.5*(v - mu)*(v - mu)/(sigma*sigma))*((v - mu)*(v - mu)/pow(sigma, 4) - 1.0/(sigma*sigma))+
		//exp(-0.5*(v + mu)*(v + mu)/(sigma*sigma))*((v + mu)*(v + mu)/pow(sigma, 4) - 1.0/(sigma*sigma));	// Sum of Gaussians
	//return (8*v*v - 2*(1+v*v))/((1+v*v)*(1+v*v)*(1+v*v));
}

void translation()
{
	totalTranslations++;
	double delta = generator->Rannyu(-delta_translation,delta_translation);
	...
	if(generator->Rannyu()<acceptance_probability)
	{
		for(int i=0;i<timeslices;i++)
			positions[i]+=delta;
...
void brownianBridge()
{
	...
	int starting_point = (int)(generator->Rannyu()*available_starting_points);
	...
	for(int i=0;i<brownianBridgeReconstructions;i++)
	{
		...
		double newcoordinate = generator->Gauss(average_position,sqrt(variance));
...
void brownianMotion(int which) // BM is called only for PIGS simulations
{
	int starting_point, endpoint, left_reco;
		...
		starting_coord = generator->Gauss(average_position,sqrt(variance));
		...
		starting_coord = positions[starting_point];
                ...
                variance = 2*lambda*dtau*(brownianMotionReconstructions+1);
		ending_coord = generator->Gauss(average_position,sqrt(variance));
...
{
	ofstream out("Output_PIGS/harmonic.const.dat");
	for(int i=0;i<timeslices;i++)
	{
...
{
	ofstream out("Output_PIGS/kinetic.var.dat");
	for(int i=0;i<timeslices;i++)
	{
...
{
	ofstream out("Output_PIMC/probability."+to_string(nconf)+".dat");
	//ofstream temp("Output_PIMC/temperatures.dat", ios::app)
	out << "Temperature: " << temperature << endl<< endl;
...
void readInput()
{
	...
	double t = 0, bb = 0, del = 0;
	input_file >> string_away >> timeslices;
	input_file >> string_away >> t;
	temperature = t - (nconf-1)/(double)10;
	input_file >> string_away >> imaginaryTimePropagation;
	input_file >> string_away >> brownianMotionReconstructions;
	input_file >> string_away >> del;
	delta_translation = del - (nconf)/(double)12;
	input_file >> string_away >> bb;
	brownianBridgeReconstructions = bb + (12.5-nconf)*5;
